<template>
	<div class="exhibitorsReg conference">
		...
		<div class="content">
			<div class="t-equity">
				<div class="t-e-text">入场券权益</div>
				<div class="t-e-con">
					<div class="t-e-time">2019年11月28日-30日 全时段</div>
					<div class="t-e-info">
						<span class="t-e-tit">线上</span>
						<span>亚太金融小镇移动端展示</span>
					</div>
					<div class="t-e-info">
						<span class="t-e-tit">线下</span>
						<span>自由出入会议展厅、休息室<br />午宴、咖啡厅、会议室、研讨会<br />峰会、晚宴、社交<br />金融10项全能精英赛</span>
					</div>
				</div>
			...
			<div class="entInfo mb-15">
				...
				<ul>
					<li><span class="info-t">企业名称</span><input type="text" placeholder="必填" v-model.trim="entInfo.company"></li>
					<li class="up_logo">
					...
					<li><span class="info-t">详细地址</span><input type="text" placeholder="选填" v-model.trim="entInfo.address"></li>
					<li>
			...
			<div class="perInfo">
				...
				<div style="padding: 0 .17rem;">
					...
					<div class="add" @click="add_per()" :disabled="disable">
						<img src="../../../../static/img/act/ee2019/add_icon.png" alt class="img-w12">添加更多参展人员
					</div>
				</div>
			</div>
			...
			submit(str) {
				...
				if (!this.mobileError && !this.nameError && !this.cityError && !this.companyError) {
					...
					this.$axios.post(this.$root.urlPath.NEW + '/ee/participants/submit',
						...
						if (res.data.success) {
							if (str == 'add') {
								window.location.href = this.$root.urlPath.M_APF + '/ee2019/addPer?type=1';
							} else {
...
<style>
	...
	.t-equity .t-e-con {
		padding: 0 .1rem .16rem .76rem;
	}

	.t-equity .t-e-con .t-e-time {
		font-size: .11rem;
		color: #707070;
		text-align: right;
		padding: .07rem 0;
