<template>
	<div class="exhibitorsList conference">
		<div class="content">
			<div class="perInfo">
				<div class="tit">{{type==1? '参会': '参展'}}人员信息列表</div>
					...
					<ul id="ul">
						<li v-for="item in perList" :key="item.id">
							...
							<div class="dis-fl aic jcSb"><span class="fs-13">{{item.position}}</span><span><span class="btn btn-modify"
									 @click="modify(item.id)">修改</span><span class="btn btn-del" @click="del(item.id)">删除</span></span></div>
			...
			<div class="complete" @click="submit()" :disabled="disable">提交</div>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			modify(id) { // 修改
				window.location.href = this.$root.urlPath.M_APF + '/ee2019/addPer?operate=modify&type=' + this.type + '&id=' + id;
			},
			del(id) { // 删除
				MessageBox.confirm('确定要删除此成员吗？').then(action => {
					const PARAMS = {
						id: id,
			...
			add_per() {
				window.location.href = this.$root.urlPath.M_APF + '/ee2019/addPer?type=' + this.type;
			},
			submit() {
				const PARAMS = {
					APF_UID: getCookie('APF_UID'),
				...
				this.$axios.post(this.$root.urlPath.NEW + '/ee/participants/submission', // 完成提交参会参展信息
					...
					if (res.data.success) {
						window.location.href = this.$root.urlPath.M_APF + '/ee2019/subDetails?type=' + this.type;
					} else {
						this.$layer.msg(res.data.errMsg);
					}
				}).catch(err => {
