			...
			<div class="con">
				<div class="form">
					<ul class="clearfix mb-20">
						...
						<li class="clearfix"><span class="w1">意向教室</span><span class="fs-14 col-70">{{selected}}</span><img class="fr"
							 src="../../../../static/img/act/20181008/right.png" alt="" style="width: 0.1rem;vertical-align: inherit;" />
							<select name="" v-model="selected" @click="change()" id="select">
								<option v-for="option in options" :value="option.value" :key="option.value">{{option.text}}</option>
							</select>
						</li>
						<li class="clearfix"><span class="w1">意向场地<i class="optional">(选填)</i></span><span class="fs-14 col-70">{{selected2}}</span><img
							 class="fr" src="../../../../static/img/act/20181008/right.png" alt="" style="width: 0.1rem;" />
						...
						<li class="clearfix"><span class="fl w1">单位</span><input type="text" placeholder="请输入单位名称" class="fr"
							 v-model.trim="userinfo.unitName" /></li>
						...
						<li class="clearfix"><span class="fl w1">预计参与培训人数</span><input type="text" placeholder="请输入培训人数" class="fr"
							 v-model.trim="userinfo.peoNum" /></li>
						<li class="clearfix"><span class="fl w1">预计需要宿舍房间</span><input type="text" placeholder="请输入房间数量" class="fr"
							 v-model.trim="userinfo.roomsNum" /></li>
					</ul>
...
<script>
	...
	export default {
		...
		methods: {
			...
			signUp() {
				var num = /^[123456789]/;
				this.roomsNumError = this.userinfo.roomsNum.length > 0 && this.userinfo.roomsNum > 0 && this.userinfo.roomsNum <
					151 && num.test(this.userinfo.roomsNum) ? false : true;
				...
				this.peoNumError = this.userinfo.peoNum.length > 0 && this.userinfo.peoNum > 0 && this.userinfo.peoNum < 301 &&
					/^[123456789]/.test(this.userinfo.peoNum) ? false : true;
				if (this.peoNumError) {
				...
				this.unitNameError = this.userinfo.unitName.length > 0 && /^[\u4e00-\u9fa5A-Za-z]{1,20}$/.test(this.userinfo.unitName) ?
					false : true;
				...
				if (!this.dataError && !this.roomsNumError && !this.peoNumError && !this.emailError && !this.telError && !this.telError &&
					!this.unitNameError && !this.nameError && !this.selectedError) {
					this.disable = true;
