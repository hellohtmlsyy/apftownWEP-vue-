			...
			<div class="con">
				<div class="form">
					<ul class="clearfix">
						...
						<li class="clearfix"><span class="w1">预订别墅数量</span><span class="fs-30 col-333" style="width:42%;text-align: right;">{{selected =='点击选择'? selected: (selected + '栋')}}</span><img class="fr" src="../../../../static/img/act/20181008/right.png" alt="" style="width: 0.1rem;vertical-align: inherit;" />
						...
						<li class="clearfix" style="padding: 0.1rem 0;"><span class="fl w1" style="margin-top: 0.05rem;">是否需要旅游服务</span><mt-switch class="fr" v-model="userinfo.tourism"></mt-switch></li>
						<li class="clearfix" style="padding: 0.1rem 0;"><span class="fl w1" style="margin-top: 0.05rem;">是否需要接送服务</span><mt-switch class="fr" v-model="userinfo.relay"></mt-switch></li>
	...
	export default {
		...
		methods: {
			...
			signUp() {
				...
				this.dataError = this.selected1Date >=dateFormat(new Date(), "yyyy-MM-dd") && this.toDate > this.fromDate ? false :true;
				if(this.dataError){
					...
					.then(res=>{
						if(res.data.data!=false){
							...
							const params = {
								...
								additional2: data,
								additional3: (this.lunch && this.dinner ? '1,1':(this.lunch && !this.dinner ? '1,0':(!this.lunch && this.dinner ? '0,1': '0,0'))),
								additional4: (this.userinfo.tourism && this.userinfo.relay ? '1,1':(this.userinfo.tourism && !this.userinfo.relay ? '1,0':(!this.userinfo.tourism && this.userinfo.relay ? '0,1': '0,0'))),
