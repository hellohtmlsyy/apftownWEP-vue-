<template>
	<div class="addPer conference">
		<div class="content">
			<div class="perInfo">
				<div class="tit">
					<a :href="('/ee2019/perList_en?type='+ type)"><img src="../../../../static/img/act/ee2019/left.png" alt="" class="img-w10"></a>
					{{tit}}
				</div>
				<div style="padding: 0 .17rem;">
					<ul>
						...
						<li>
							...
							<mt-radio v-model="userInfo.gender" :options="['Mr.', 'Ms.']"></mt-radio>
						</li>
			...
			<div class="complete" @click="submit()" :disabled="disable">OK</div>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			submit(str) {
				...
				this.positionError = this.userInfo.position == null || /^.{0,20}$/.test(this.userInfo.position) ? false : true;
				if (this.positionError) {
					this.$layer.msg('Please enter the correct position');
					return;
				}
				this.givenNameError = /^.{1,20}$/.test(this.userInfo.givenName) ? false : true;
				if (this.givenNameError) {
					this.$layer.msg('Please enter the correct given name');
				}
				this.surNameError = /^.{1,10}$/.test(this.userInfo.surname) ? false : true;
				if (this.surNameError) {
					this.$layer.msg('Please enter the correct surname');
				}
				if (!this.mobileError && !this.givenNameError && !this.surNameError) {
					this.disable = true;
					const PARAMS = {
						serialNo: '20191131',
						surname: this.userInfo.surname,
						givenName: this.userInfo.givenName,
						gender: this.userInfo.gender == 'Mr.' ? '1' : '0',
					...
					if (this.operate || this.operate == 'modify') {
						...
						this.$axios.post(this.$root.urlPath.NEW + '/ee/participants/update',
							...
							if (res.data.success) {
								window.location.href = this.$root.urlPath.M_APF + '/ee2019/perList_en?type=' + this.$route.query.type;
							} else {
						...
						this.$axios.post(this.$root.urlPath.NEW + '/ee/participants/add',
							...
							if (res.data.success) {
								if (str == 'add') {
									// 添加人员
									window.location.href = this.$root.urlPath.M_APF + '/ee2019/addPer_en?type=' + this.$route.query.type;
								} else {
									// 提交表单
									window.location.href = this.$root.urlPath.M_APF + '/ee2019/perList_en?type=' + this.$route.query.type;
								}
			...
			perInfo() {
				this.$axios.get(this.$root.urlPath.NEW + '/ee/participants/info', { // 获取参会参展人员信息详情
					...
					if (res.data.success) {
						...
						this.userInfo.gender = res.data.data.gender == 1 ? 'Mr.' : 'Ms.';
					} else {
						this.$layer.msg(res.data.errMsg);
					}
				}).catch(err => {
		...
		mounted() {
			...
			if (this.operate || this.operate == 'modify') {
				this.tit = 'Modify personnel information';
				this.isAdd = false;
				this.perInfo();
			} else {
				this.tit = 'Add a new person';
			}
		},
	}
</script>
