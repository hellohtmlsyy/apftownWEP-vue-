<template>
	<div class="exhibitorsReg conference">
		...
		<div class="content">
			<div class="t-equity">
				<div class="t-e-top">Participants' Rights And Interests</div>
				<div class="t-e-con">
					<div class="t-e-info">
						<p class="t-e-tit mb-15">Online</p>
						<p class="t-e-tit">Offline</p>
					</div>
					<div class="t-e-right">
						<p class="mb-15">Advertising online</p>
						<p class="mb-5">Free access to Exhibition hall, Lounge hall,Cafe, Meeting room, Submit, Symposium,Luncheon,
							Dinner during all opening hours on the conference.</p>
						<p>International Finance Decathlon Elite Competition.</p>
					</div>
				</div>
				<div class="t-e-bot">November 28th - 30th, 2019 Full time interval</div>
				<span class="circle"></span>
			</div>
			<div class="entInfo mb-15">
				<div class="tit">Enterprise information</div>
				<ul>
					<li><span class="info-t">Company name</span><input type="text" placeholder="Required" v-model.trim="entInfo.company"></li>
					<li class="up_logo">
					...
					<li><span class="info-t">City</span><input type="text" placeholder="Optional" v-model.trim="entInfo.city"></li>
					<li><span class="info-t">Address</span><input type="text" placeholder="Optional" v-model.trim="entInfo.address"></li>
					<li>
			...
			<div class="perInfo">
				...
				<div style="padding: 0 .17rem;">
					...
					<div class="add" @click="add_per()" :disabled="disable">
						<img src="../../../../static/img/act/ee2019/add_icon.png" alt class="img-w12">Add a new person
					</div>
				</div>
			</div>
			...
			submit(str) {
				...
				if (!this.mobileError && !this.surNameError && !this.givenNameError && !this.companyError) {
					...
					this.$axios.post(this.$root.urlPath.NEW + '/ee/participants/submit',
						...
						if (res.data.success) {
							if (str == 'add') {
								window.location.href = this.$root.urlPath.M_APF + '/ee2019/addPer_en?type=1';
							} else {
...
<style>
	...
	.t-equity {
		...
		box-shadow: 0px 0px .05rem 0px #1b1b1b;
		border-radius: .1rem;
		min-height: 1.5rem;
		margin-bottom: .15rem;
	}
	...
	.t-equity .t-e-info .t-e-tit {
		background-color: #e93a19;
		border-radius: .08rem;
		color: #fff;
		padding: .01rem .09rem;
	}
