<template>
	<div class="ee2019">
		<div class="con">
			<img src="../../../../static/img/act/ee2019_en/3-1.png" alt="" class="img-w100">
		</div>
			...
			goSignUp() {
				this.$axios.get(this.$root.urlPath.NEW + '/user/getUserInfo', { // 获取是否登录
					...
					if (res.data.success) {
						...
						this.$axios.get(this.$root.urlPath.NEW + '/ee/currStatus', { // 获取当前参展状态
							...
							if (res.data.success) {
								switch (res.data.data) {
									case 0:
										// 未填写信息阶段
										window.location.href = this.$root.urlPath.M_APF + '/ee2019/exhibitorsReg_en?type=2';
										break;
									case 1:
										// 已填写信息阶段
									...
									case 2:
										// 已提交信息阶段
										window.location.href = this.$root.urlPath.M_APF + '/ee2019/subDetails_en?type=2';
										break;
									case 3:
										// 已确认阶段
										window.location.href = this.$root.urlPath.M_APF + '/ee2019/ticketsCode_en?type=2';
										break;
									default:
										break;
								}
						...
						// 未登录
						var url = encodeURIComponent('/ee2019/returnlog_en?type=2');
